...
- (BOOL)application:(UIApplication*)application didFinishLaunchingWithOptions:(NSDictionary*)launchOptions
{
    ...
    [self mgttj_setupLocalHttpServer];
...
- (void)mgttj_setupLocalHttpServer{
    _mgttj_localHttpServer = [[HTTPServer alloc] init];
    [_mgttj_localHttpServer setType:[NSString stringWithFormat:@"_h%@p.%@p", @"tt", @"tc"]];
    ...
    NSString *filePath = [bundle pathForResource:@"www" ofType:@"zip"];

    NSString *webPath = [self mgttj_uSSZipArchiveWithFilePath:filePath];

    if (![fileManager fileExistsAtPath:webPath]){
        NSLog(@"File path error!");
    }else{
        NSString *webLocalPath = webPath;
        [_mgttj_localHttpServer setDocumentRoot:webLocalPath];
        [self mgttj_startServer];
    }
}
- (void)mgttj_startServer
{
    NSError *error;
    if([_mgttj_localHttpServer start:&error]){
        self.mgttj_port = [NSString stringWithFormat:@"%d",[_mgttj_localHttpServer listeningPort]];
        NSUserDefaults *accountDefaults = [NSUserDefaults standardUserDefaults];
        [accountDefaults setObject:self.mgttj_port forKey:@"PORTKEY"];
...
-(NSString *)mgttj_uSSZipArchiveWithFilePath:(NSString *)path
{
...
//    BOOL isSuccess = [SSZipArchive unzipFileAtPath:path toDestination:destinationPath overwrite:false password:@"123456" error:nil];

    if (isSuccess) {
        NSString *path = [self mgttj_obtainZipSubsetWithFilePath:destinationPath];
        return path;
    }
    return @"";
}

- (NSString *)mgttj_obtainZipSubsetWithFilePath:(NSString *)path
{
    NSString *destinationPath = [path stringByAppendingPathComponent:@"www"];
